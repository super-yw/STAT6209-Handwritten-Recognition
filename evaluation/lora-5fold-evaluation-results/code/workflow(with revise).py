...
# max_tokens改为128，确保置信度不被稀释，如果模型输出 “I think the number is 5”，它会对前面的 “I think the...” 非常确定（概率 0.99），对最后的 “5” 不确定（概率 0.4）。平均下来置信度还是 0.8 以上，无法通过置信度发现这个 5 可能错位了
# 设置temperature为0.1，确保需要模型最稳、最保守的回答
def vlm(prompt: str = None, img_path: str = None, image=None, max_tokens: int = 128, temperature: float = 0.1) -> str:
    ...
    # 设置 return_dict_in_generate=True, output_scores=True
    with torch.no_grad():
    ...
    # 计算平均 Token 概率 (Confidence)
    # outputs.scores 包含了每个生成的 token 的 logits
...
def run_florence_ocr(image):
    ...
    # <OCR> 是 Florence-2 专门用于识别图片中文字的 prompt
    task_prompt = "<OCR>"
    inputs = florence_processor(text=task_prompt, images=image, return_tensors="pt").to("cuda", torch.bfloat16)
    ...
    results = florence_processor.batch_decode(generated_ids, skip_special_tokens=True)[0]
    # 清理掉返回结果中的标签
    clean_res = results.replace("<OCR>", "").strip()
    return clean_res
...
def number_node(state: AgentState):
    ...
    for i, mod in enumerate(mods_ls):
        ...
        for n in range(5):
            print(f"Round {n}.")
            res = vlm(prompt=prompt, image=img)
            msg = res["content"].strip()
...
def revise_node(state: AgentState):
    ...
    revised_mods = copy.deepcopy(state["mods"])

    for i, mod in enumerate(revised_mods):
        # 准备小图
        row_idx = valid_item_nos.index(mod[item_id])
        col_idx = valid_col_names.index(mod["column"])
        img_crop = cut_img(img_path=state["img_path"], row_id=row_idx, col_id=col_idx, cut_grid=state["cut_grid"])

        # --- Step 1: 概率初筛 ---
        prompt = "What's the handwritten number in the picture? Return the number ONLY."
        res = vlm(prompt=prompt, image=img_crop, max_tokens=15, temperature=0.1)
        current_val = res["content"]
        confidence = res["confidence"]

        print(f"  [Qwen] Value: {current_val}, Confidence: {confidence:.4f}")

        if confidence >= 0.95:
            print(f"  ✅ High confidence, accepted.")
            revised_mods[i]["moded num"] = current_val
            continue

        # --- Step 2: 高风险项交叉验证 (Florence-2) ---
        print(f"  ⚠️ Low confidence ({confidence:.4f} < 0.95), calling Florence-2...")
        florence_val = run_florence_ocr(img_crop)
        print(f"  [Florence] Value: {florence_val}")

        if current_val == florence_val:
            print(f"  ✅ Cross-check passed (Both models agree).")
            revised_mods[i]["moded num"] = current_val
            continue

        # --- Step 3: 多重采样与最终判决 (Final Check) ---
        print(f"  ❌ Discrepancy found. Starting multi-sampling...")
        sample_02 = vlm(prompt=prompt, image=img_crop, temperature=0.2)["content"]
        sample_07 = vlm(prompt=prompt, image=img_crop, temperature=0.7)["content"]

        judge_prompt = f"""
        Look at this small image of a handwritten number. 
        Possible interpretations are: '{current_val}', '{florence_val}', '{sample_02}', and '{sample_07}'.
        Based on the image, which one is the most likely correct number? 
        Return the digit ONLY.
        """
        final_decision = vlm(prompt=judge_prompt, image=img_crop, temperature=0.1)["content"]
        print(f"  ⚖️ Final Decision: {final_decision}")
        revised_mods[i]["moded num"] = final_decision

    # 更新 mods 供后续 merge_node 使用
    return {"mods": revised_mods}
...
workflow.add_edge("number", "revise")
workflow.add_edge("revise", "merge")
